...
public:
	...
	virtual bool hit(const Ray& ray, float& tmin, ShadeRec& out_hit_info) override
	{
		...
		if (a >= 0)
			return false;
		float b = dot(this->normal, ray.origin - this->position);
...
public:
	...
	void set_center(const Vec3& center) { this->center = center; };
	void set_radius(const float radius) { this->radius = radius; };

	virtual bool hit(const Ray& ray, float& tmin, ShadeRec& out_hit_info) override
	{
		auto v = ray.origin - center;
		float a0 = v.square_length() - radius * radius;
		...
		if (DdotV <= 0)
		{
			...
			if (disc_r >= 0) {
				...
				auto position = ray.point_at(tmin);
